...
from file_processing.json_to_chroma_data import data_to_save_request
...
async def fill_rag_db(encoding_model: str = Query('openai', enum=('gigachat', 'local_all_12', 'openai'))):
    ...
    if current_size == 0:
        file_path = '/opt/app-root/tinkoff_data/dataset.json'
        request = await data_to_save_request(encoding_model, file_path)
