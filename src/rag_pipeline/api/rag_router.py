...
async def get_rag_db_response(
    ...
    res = await rag_db_response(request, encoding_model, n_results, include_embeddings)
    return res
...
async def get_rag_final_response(
    ...
    res = await rag_final_response(request, encoding_model, n_results, include_embeddings)
    return res
...
async def get_rag_final_response(
    ...
    res = await rag_final_response(request, encoding_model='openai', n_results=5, include_embeddings=False)
    return res
